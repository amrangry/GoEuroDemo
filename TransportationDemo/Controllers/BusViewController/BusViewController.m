...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [TransportationData getDataForURLString:@"https://api.myjson.com/bins/37yzm" onSuccess:^(NSMutableArray *results) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TransportationTableViewCell *cell=[busTableView dequeueReusableCellWithIdentifier:CellIdentifier];
  //  NSLog(@" in 2 ");
...
-(void)setRandomImageForCellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TransportationTableViewCell *cell =  [busTableView cellForRowAtIndexPath:indexPath];
...
bool isPriceOrderAsecending=NO;
- (IBAction)priceBtnPressed:(id)sender {
    
    
    [self setFocusButtonFroSorting:3];
    isPriceOrderAsecending=!isPriceOrderAsecending;
    
    
    if (isPriceOrderAsecending) {
    ...
    NSSortDescriptor* sortOrder = [NSSortDescriptor sortDescriptorWithKey: @"price_in_euros"
                                                                            ascending: isPriceOrderAsecending];
...
bool isDeparturalTimeOrederAscending=NO;
- (IBAction)departureTimeBtnPressed:(id)sender {
    
    [self setFocusButtonFroSorting:1];
    isDeparturalTimeOrederAscending=!isDeparturalTimeOrederAscending;
    
    
    if (isDeparturalTimeOrederAscending) {
        [sender setBackgroundImage:[UIImage imageNamed:@"arrow_down"]forState:UIControlStateNormal];
    ...
    NSSortDescriptor* sortOrder = [NSSortDescriptor sortDescriptorWithKey: @"departualDate"
                                                                ascending: isDeparturalTimeOrederAscending];
    NSArray * sortedArray=  [busArray sortedArrayUsingDescriptors: [NSArray arrayWithObject: sortOrder]];
    busArray=[NSMutableArray arrayWithArray:sortedArray];
    super.arrayOfResults=busArray;
    [busTableView reloadData];
...
bool isArrivalTimeOrderAscending=NO;
- (IBAction)arrivalTimeBtnPressed:(id)sender {
    
    [self setFocusButtonFroSorting:2];
    isArrivalTimeOrderAscending=!isArrivalTimeOrderAscending;
    
    if (isArrivalTimeOrderAscending) {
        [sender setBackgroundImage:[UIImage imageNamed:@"arrow_down"]forState:UIControlStateNormal];
    ...
    NSSortDescriptor* sortOrder = [NSSortDescriptor sortDescriptorWithKey: @"arrivalDate"
                                                                ascending: isArrivalTimeOrderAscending];
    NSArray * sortedArray=  [busArray sortedArrayUsingDescriptors: [NSArray arrayWithObject: sortOrder]];
    busArray=[NSMutableArray arrayWithArray:sortedArray];
    super.arrayOfResults=busArray;
    [busTableView reloadData];
