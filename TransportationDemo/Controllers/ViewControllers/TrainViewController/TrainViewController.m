...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [TransportationData getDataForURLString:@"https://api.myjson.com/bins/3zmcy" onSuccess:^(NSMutableArray *results) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TransportationTableViewCell *cell=[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    //  NSLog(@" in 2 ");
...
bool isPriceOrderAsecendingTrain=NO;
- (IBAction)priceBtnPressed:(id)sender {
    
    
    [self setFocusButtonFroSorting:3];
    isPriceOrderAsecendingTrain=!isPriceOrderAsecendingTrain;
    
    
    if (isPriceOrderAsecendingTrain) {
    ...
    NSSortDescriptor* sortOrder = [NSSortDescriptor sortDescriptorWithKey: @"price_in_euros"
                                                                ascending: isPriceOrderAsecendingTrain];
...
bool isDeparturalTimeOrederAscendingTrain=NO;
- (IBAction)departureTimeBtnPressed:(id)sender {
    
    [self setFocusButtonFroSorting:1];
    isDeparturalTimeOrederAscendingTrain=!isDeparturalTimeOrederAscendingTrain;
    
    
    if (isDeparturalTimeOrederAscendingTrain) {
        [sender setBackgroundImage:[UIImage imageNamed:@"arrow_down"]forState:UIControlStateNormal];
    ...
    NSSortDescriptor* sortOrder = [NSSortDescriptor sortDescriptorWithKey: @"departualDate"
                                                                ascending: isDeparturalTimeOrederAscendingTrain];
    NSArray * sortedArray=  [busArray sortedArrayUsingDescriptors: [NSArray arrayWithObject: sortOrder]];
    busArray=[NSMutableArray arrayWithArray:sortedArray];
    super.arrayOfResults=busArray;
    [tableViewRef reloadData];
...
bool isArrivalTimeOrderAscendingTrain=NO;
- (IBAction)arrivalTimeBtnPressed:(id)sender {
    
    [self setFocusButtonFroSorting:2];
    isArrivalTimeOrderAscendingTrain=!isArrivalTimeOrderAscendingTrain;
    
    if (isArrivalTimeOrderAscendingTrain) {
        [sender setBackgroundImage:[UIImage imageNamed:@"arrow_down"]forState:UIControlStateNormal];
    ...
    NSSortDescriptor* sortOrder = [NSSortDescriptor sortDescriptorWithKey: @"arrivalDate"
                                                                ascending: isArrivalTimeOrderAscendingTrain];
    NSArray * sortedArray=  [busArray sortedArrayUsingDescriptors: [NSArray arrayWithObject: sortOrder]];
    busArray=[NSMutableArray arrayWithArray:sortedArray];
    super.arrayOfResults=busArray;
    [tableViewRef reloadData];
